...
def convert_json_to_xml(input_json_filename, output_xml_filename):
    ...
    with open(input_json_filename, 'r', encoding='UTF-8') as json_file:
        json_data_root = json.load(json_file)
    ...
    with open(output_xml_filename, 'w', encoding='UTF-8') as f_out:
        ...
        # 处理每个顶层对象
        for top_level_obj in json_data_root:
            archive_list = top_level_obj.get('archive', [])
            if not archive_list:  # 跳过空的archive列表
                continue
            
            # 处理每条评论事件
            for comment_event in archive_list:
